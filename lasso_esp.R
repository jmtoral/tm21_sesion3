...
pp <- readtext::readtext("literatura/detectives.pdf") %>% 
  as_tibble() %>% 
  unnest_tokens(oracion, text, token = "sentences")

cronopio <- readtext::readtext("literatura/cronopios_famas.pdf")%>% 
  as_tibble() %>% 
  unnest_tokens(oracion, text, token = "sentences")
...
is_jane <- books_joined$title == "cronopios_famas.pdf"
...
comment_classes %>%
  mutate(
    prediction = case_when(
      probability > 0.5 ~ "detectives.pdf",
      TRUE ~ "cronopios_famas.pdf"
    ),
...
comment_classes %>%
  filter(
    probability > .8,
    title == "detectives.pdf"
  ) %>%
  sample_n(10) %>%
  inner_join(books %>%
               select(document, oracion)) %>%
  select(probability, oracion)
